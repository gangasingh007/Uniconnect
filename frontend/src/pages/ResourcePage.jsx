...
import { 
  ...
  DownloadCloud // Added for Download button
} from 'lucide-react';
                        ...
                        <div className="flex border-b border-white/10 bg-black/20 backdrop-blur-sm rounded-t-xl overflow-hidden">
                            {[{ key: 'Yt-Link', label: 'YouTube Links', icon: Youtube, count: ytLinks.length }, { key: 'Document', label: 'Documents', icon: FileText, count: documents.length }].map((tab) => (
                                <motion.button key={tab.key} onClick={() => setActiveTab(tab.key)} className={`px-4 sm:px-6 py-4 font-semibold transition-colors duration-300 relative group flex-1 sm:flex-none ${activeTab === tab.key ? 'text-white' : 'text-gray-400 hover:text-white'} flex items-center justify-center gap-3`} whileTap={{ scale: 0.98 }}>
                                    <tab.icon size={18} /><span className="hidden sm:inline">{tab.label}</span><span className={`px-2 py-0.5 rounded-full text-xs font-bold ${activeTab === tab.key ? 'bg-purple-500/20 text-purple-300' : 'bg-gray-500/20 text-gray-400'}`}>{tab.count}</span>
...
const ResourceRow = ({ resource, isAdmin, onEdit, onDelete }) => {
    ...
    const viewUrl = resource.link?.replace('/upload/', '/upload/fl_inline/');
    const downloadUrl = resource.link?.replace('/upload/', '/upload/fl_attachment/');
            ...
            <div className="col-span-12 md:col-span-3 flex items-center justify-end gap-2">
                <div className="flex items-center gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-300">
                    {resource.type === 'Document' && (
                        <>
                            <a href={viewUrl} target="_blank" rel="noopener noreferrer" className="p-2 bg-green-500/20 hover:bg-green-500/40 text-green-300 rounded-full" title="View PDF"><Eye size={16} /></a>
                            <a href={downloadUrl} download className="p-2 bg-purple-500/20 hover:bg-purple-500/40 text-purple-300 rounded-full" title="Download PDF"><DownloadCloud size={16} /></a>
                            <button onClick={(e) => { e.preventDefault(); toast('AI Summarization coming soon!'); }} className="p-2 bg-yellow-500/20 hover:bg-yellow-500/40 text-yellow-300 rounded-full" title="Summarize (AI)"><Sparkles size={16} /></button>
                        </>
                    )}
                    {isAdmin && (
                        <>
                            <button onClick={(e) => { e.preventDefault(); onEdit(resource); }} className="p-2 bg-blue-500/20 hover:bg-blue-500/40 text-blue-300 rounded-full" aria-label="Edit"><Pencil size={16} /></button>
                            <button onClick={(e) => { e.preventDefault(); onDelete(resource); }} className="p-2 bg-red-500/20 hover:bg-red-500/40 text-red-300 rounded-full" aria-label="Delete"><Trash2 size={16} /></button>
                        </>
                    )}
                </div>
                {resource.type === 'Yt-Link' && (
                    <a href={resource.link} target="_blank" rel="noopener noreferrer" className="p-2 text-gray-400 hover:text-white transition-colors" aria-label="Open Link"><ExternalLink size={18} /></a>
                )}
            </div>
        </motion.div>
    );
...
const FormModal = ({
    ...
    >
        <motion.div
            className="bg-gradient-to-br from-[#1f1f35] to-[#1a1a2e] backdrop-blur-xl rounded-3xl shadow-2xl max-w-lg w-full border border-white/10 overflow-hidden"
        ...
        >
            ...
            <div className="p-8 border-b border-white/10 relative">
                <h2 className="text-2xl font-bold bg-gradient-to-r from-pink-400 to-purple-400 bg-clip-text text-transparent">
                    {resourceToEdit
                        ? 'Update Resource'
                        : `Add New ${activeTab === 'Yt-Link' ? 'YouTube Link' : 'Document'}`}
                </h2>
                <motion.button
                    onClick={onClose}
                    className="absolute top-6 right-6 text-gray-400 hover:text-white p-2 rounded-full hover:bg-white/10 transition-all"
            ...
            <form onSubmit={onSubmit} className="p-8 space-y-6">
                ...
                {(resourceToEdit?.type || activeTab) === 'Yt-Link' ? (
                    ...
                    <FileInput
                        ...
                        existingFileName={resourceToEdit?.title}
                    />
                )}
                ...
                <div className="flex gap-4 pt-4">
                    <motion.button
                        type="button"
                        onClick={onClose}
                        className="flex-1 py-3 text-gray-300 bg-white/10 hover:bg-white/20 rounded-xl font-semibold transition-all"
